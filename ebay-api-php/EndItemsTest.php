...
include_once('comm.inc'); //共通ファイルを取り込みます。
  ...
  //ヘッダ用データを生成します
  $hdrs = createHeaders('EndItems');

  //入力データのXMLファイルを読み込みます。
  $body = file_get_contents('xml/case11.xml');
  ...
  if ($xml->Ack == 'Success' || $xml->Ack=='PartialFailure') 
  {
    ...
    //全て成功または一部成功の場合
    if (isset($xml->EndItemResponseContainer))
    {
      //１件ずつ処理結果を確認
      foreach($xml->EndItemResponseContainer as $cnt) 
      {
